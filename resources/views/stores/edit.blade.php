                ...
                <form method="POST" action="{{ route('stores.update', $store) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="type" class="w-full border rounded p-2" required>
                            <option value="main" @selected(old('type', $store->type)==='main')>Main</option>
                            <option value="sub" @selected(old('type', $store->type)==='sub')>Sub</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Description</label>
                        <textarea name="description" class="w-full border rounded p-2" rows="3">{{ old('description', $store->description) }}</textarea>
                    </div>
