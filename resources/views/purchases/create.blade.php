<x-app-layout>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('purchases.store') }}" method="POST" class="space-y-4">
                    @csrf
                    @php
                        // Pre-compute root unit id for each unit to enable fast client-side filtering
                        $unitRootIds = [];
                        foreach ($units as $u) {
                            $anc = $u->ancestry();
                            $root = end($anc);
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input name="supplier_name" class="w-full border rounded p-2" required />
                        </div>
                        <div>
                            <label class="block text-sm font-medium">Supplier Phone</label>
                            <input name="supplier_phone" class="w-full border rounded p-2" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium">Date</label>
                            <input type="date" name="purchase_date" class="w-full border rounded p-2" required value="{{ old('purchase_date', now()->format('Y-m-d')) }}" />
                        </div>
                    </div>
            ...
            <div id="items" class="space-y-4">
                ...
                <select name="items[0][inventory_item_id]" class="border rounded p-2 item-select" required>
                            ...
                            <input type="number" step="0.01" name="items[0][unit_price]" placeholder="Unit Price" class="border rounded p-2" required />
                            <select name="items[0][to_store_id]" class="border rounded p-2" required>
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('purchases.index') }}" class="px-4 py-2 rounded border">Cancel</a>
    ...
    <script>
        // Build a JS array of all units with their root ids for quick filtering
        @php
            $unitsForJs = [];
            foreach ($units as $u) {
                $unitsForJs[] = [
    ...
    const UNITS = @json($unitsForJs);
        // Build a JS array of inventory items (id, name, unit label, unit root)
        @php
            $inventoryList = [];
            foreach ($inventoryItems as $it) {
                $anc = $it->unit ? $it->unit->ancestry() : [];
        ...
        function populateUnitOptions(unitSelect, rootId) {
            // Preserve current value if still allowed
        ...
        document.getElementById('add-item').addEventListener('click', () => {
            ...
            wrap.insertAdjacentHTML('beforeend', `
                <div class="item grid grid-cols-1 md:grid-cols-5 gap-2">
                    ...
                    <input type="number" step="0.01" name="items[${i}][unit_price]" placeholder="Unit Price" class="border rounded p-2" required />
                    <select name="items[${i}][to_store_id]" class="border rounded p-2" required>
                        <option value="">Select Store</option>
                        @foreach(\App\Models\Store::orderBy('name')->get() as $st)
                            <option value="{{ $st->id }}">{{ $st->name }}</option>
                        @endforeach
            ...
            `);
            // Wire up change handler for the newly added row
        ...
        // Delegate change handler for existing first row
        document.querySelectorAll('.item-select').forEach(sel => sel.addEventListener('change', (e) => onItemChange(e.target)));
        // Initialize unit options for the first row if an item is preselected
